...
ctrl+D	导入车次	1.0.7	选择运行图文件，导入其中所有在本线2站以上的车次
ctrl+shift+D	导入实际运行线	1.0.7	选择运行图文件，将其中在本线的车次导入为“实际”类型的车次，其车次前冠以R
...
ctrl+P	线路信息	1.0.7	显示当前线路信息，包含站点总数，里程，车次数量统计等。
...
Alt+X	排图冲突检查	2.2.6	在“标尺排图向导”运行时有效，检查当前行到发时刻附近的其他列车时刻。自2.2.6版本将快捷键由Alt+C修改为Alt+X。
"""
